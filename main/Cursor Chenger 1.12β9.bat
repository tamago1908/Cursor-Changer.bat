...
rem コードの最初に入れれる処理の数は限られています(入れすぎると起動が遅くなる可能性) (遅くなった (手遅れ) )
rem ビルドナンバーとバージョンの記入は必ずしてください！あとアーカイブに入れるのを忘れずに！
rem そして最近、このバッチ処理に英語を多く含めるようにしている。なぜなら、将来的にこのバッチ処理を翻訳することになった時、日本語が多いと面倒だから。S
title カーソル変え
rem VER v1.12β9
set batver=1.12β9
set batbuild=build 40
...
rem 一番開発に時間がかかったのはバージョン1.10である。
...
rem 現在時刻を判別してあいさつをする。日付によっても挨拶を決める。
rem 全体的なコードの整備
rem 多言語対応 
rem アンインストールメニューを完成させる＃開発中
rem ファイルだったり、READMEとかも＃未定だけど形としては構想はねってある

rem ver 1.14 
rem より派手なアニメーションとsetxを用いた起動回数のカウントとそれの忠告画面

rem ver 1.15 
rem 色でわかりやすい！チュートリアルとより進化したdogcheck (samチェックのような機能を実装)

rem ver 1.16 
rem より良い初回起動時のメッセージと派手なUI、変更できるカーソルの種類を増加

rem ver 1.17 
rem より多くの設定とより良い安定したプログラム

rem ver 1.18 
rem 単体でのアップデート確認機能とより安全性を高める、カスタムカーソル機能
