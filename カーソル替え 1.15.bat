...
mode con: cols=75 lines=25
echo ©“®C•œ‚ğ—˜—p‚Å‚«‚Ü‚·I
echo ©“®C•œ‚ğÀs‚·‚é‚Æ‚»‚ê‚Ü‚Å‚Ìİ’è‚ª‚·‚×‚Ä‰Šú‰»‚³‚êAƒfƒtƒHƒ‹ƒg‚Ìİ’è‚ÉƒŠƒZƒbƒg‚³‚ê‚Ü‚·B
choice /c YN /n /m "‘±s‚µ‚Ü‚·‚©H(Y ‚Ü‚½‚Í N):"
if "%errorlevel%"=="1" (
call :Wipealldeta
echo.
echo C•œ‚ªŠ®—¹‚µ‚Ü‚µ‚½B
...
echo           I                                I[5;66HI
echo           I   !ccm1!1 ƒJ[ƒ\ƒ‹‚ğ%cursorcolor%F‚É•Ï‚¦‚é %clr2%    I[6;66HI
echo           I                                I[7;66HI
echo           I   !ccm2!2 ƒJ[ƒ\ƒ‹‚ÌƒoƒbƒNƒAƒbƒv%clr2%     I[8;66HI
echo           I                                I[9;66HI
echo           O================================O=====================O
echo.
echo.
echo               %clrgra%1~2 ‚© W,S ‚Å‘I‘ğ‚µAY,E‚ÅŒˆ’èA B,N‚ÅI—¹‚µ‚Ü‚·B%clr2%
echo.
...
if "%CursorChange_Current%"=="0" (echo [6;51H ‰½‚à‘I‘ğ[7;47H ‚³‚ê‚Ä‚¢‚Ü‚¹‚ñB)
if "%CursorChange_Current%"=="1" (
    if "%CursorChange_Exit%"=="confirm1" (
        call :CursorChange_clear
        set /p nothing=[5;13H –{“–‚ÉƒJ[ƒ\ƒ‹‚ğ%cursorcolor%F‚É•ÏX‚µ‚Ü‚·‚©H[8;13H %clrgra%^(Y,E ‚Ü‚½‚Í B,N^)%clr2%<nul
    ) else (echo [6;48H ƒJ[ƒ\ƒ‹‚ğ%cursorcolor%F[7;48H ‚É•ÏX‚Å‚«‚Ü‚·B)
)
if "%CursorChange_Current%"=="2" (echo [6;47H ƒJ[ƒ\ƒ‹‚ğƒoƒbƒN[7;48H ƒAƒbƒv‚Å‚«‚Ü‚·B)
exit /b
...
if !num!==11 if "!input!"=="1908" (echo [17;28H³‚µ‚¢ƒpƒXƒ[ƒh‚Å‚·B& timeout /t 2 /nobreak >nul& set /p nothing=[?25h<nul& set invisiblecursor=& set input=& set len=& set allcommandlock=false& set allcommands_exit=correct) else (echo [17;8HŠÔˆá‚Á‚½ƒpƒXƒ[ƒh‚Å‚·BÄ“x‚â‚è’¼‚·ê‡‚ÍAÄ‹N“®‚µ‚Ä‚­‚¾‚³‚¢B& timeout /t 3 /nobreak >nul& set input=& set len=0& set allcommandlock=true& set allcommands_exit=true)
