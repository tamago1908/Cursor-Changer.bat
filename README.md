...
Besides, both of them take a long time to run the Powershell scripts, and if we do them at startup, the startup time would be so long that it would suck, so, well, basically, we would only be adding the ability to check for updates. At this stage, I'm hoping to be able to change the settings menu to confirm or deny updates at startup, but that's just the ideal. Well, that's about it.
